...
class AuthView:
    ...
    def __init__(self):
        self._estrategia: RolStrategy = None

    def _obtener_estrategia(self, rol: str) -> RolStrategy | None:
        if rol == "docente":
            self._estrategia = DocenteStrategy()
        elif rol == "estudiante":
            self._estrategia = EstudianteStrategy()
        elif rol == "admin":
            self._estrategia = AdminStrategy()
        else:
            return None
        return self._estrategia

    def handle_auth_web(self, handler, method: str, partes: list):
        accion = partes[0]

        if accion == "login":
            if method == "GET":
                self._mostrar_login(handler)
            elif method == "POST":
                self._procesar_login(handler)
            else:
            ...
            estrategia = self._obtener_estrategia(rol)
            if not estrategia:
                send_error(handler, 404, "Ruta no encontrada")
                return
            if method == "GET":
                self._mostrar_registro(handler, estrategia)
            elif method == "POST":
                self._procesar_registro(handler, estrategia)
            else:
                send_error(handler, 405, "Método no permitido")

    # ── Login ──────────────────────────────────────────────────────────────────

    def _mostrar_login(self, handler):
        html = render_template("login.html", error=None, rol="", email="")
        send_html(handler, 200, html)

    def _procesar_login(self, handler):
    ...
    def _mostrar_registro(self, handler, estrategia: RolStrategy):
        html = render_template(estrategia.get_template_registro(), error=None, datos={})
        send_html(handler, 200, html)

    def _procesar_registro(self, handler, estrategia: RolStrategy):
        body  = read_form_body(handler)
