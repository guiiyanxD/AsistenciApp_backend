...
class InscripcionBusiness:
    ...
    def __init__(self):
        self._inscripcion_data = InscripcionData()
    ...
    def unirse_por_codigo(self, codigo: str, estudiante_id: str) -> dict:
        ...
        grupo = self._inscripcion_data.buscar_grupo_por_codigo(codigo.strip().upper())
        if not grupo:
        ...
        if grupo["cupo_maximo"] is not None:
            inscritos = self._inscripcion_data.contar_inscritos(grupo["id"])
            if inscritos >= grupo["cupo_maximo"]:
                raise ValueError("El grupo ha alcanzado su cupo máximo")

        inscripcion_previa = self._inscripcion_data.buscar_por_grupo_y_estudiante(grupo["id"], estudiante_id)
        if inscripcion_previa:
            estado = self._resolver_estado(inscripcion_previa["estado"])
            inscripcion = estado.inscribir(self._inscripcion_data, grupo["id"], estudiante_id)
        else:
            inscripcion = self._inscripcion_data.crear(grupo["id"], estudiante_id)
    ...
    def aprobar(self, inscripcion_id: str) -> dict:
        inscripcion = self._inscripcion_data.buscar_por_id(inscripcion_id)
        if not inscripcion:
            raise LookupError("Inscripción no encontrada")
        estado = self._resolver_estado(inscripcion["estado"])
        return estado.aprobar(self._inscripcion_data, inscripcion_id)
    ...
    def reprobar(self, inscripcion_id: str) -> dict:
        ...
        if not inscripcion:
            raise LookupError("Inscripción no encontrada")
        estado = self._resolver_estado(inscripcion["estado"])
        return estado.reprobar(self._inscripcion_data, inscripcion_id)
    ...
    def retirar(self, inscripcion_id: str) -> dict:
        ...
        if not inscripcion:
            raise LookupError("Inscripción no encontrada")
        estado = self._resolver_estado(inscripcion["estado"])
        return estado.retirar(self._inscripcion_data, inscripcion_id)

    def listar_por_grupo(self, grupo_id: str) -> list[dict]:
        return self._inscripcion_data.listar_por_grupo(grupo_id)

    def listar_grupos_estudiante(self, estudiante_id: str) -> list[dict]:
        return self._inscripcion_data.listar_por_estudiante(estudiante_id)
