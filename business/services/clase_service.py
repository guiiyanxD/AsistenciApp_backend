...
class ClaseService:
    ...
    # ------------------------------------------------------------------
    # Guardia de acceso — verifica que la clase le pertenece al docente
    # ------------------------------------------------------------------

    def _obtener_y_verificar(self, clase_id: str,
                              docente_id: str) -> dict:
    ...
    def cambiar_tipo(self, clase_id: str, docente_id: str,
                     tipo: str) -> dict:
        ...
        if clase["cancelada"]:
            raise ValueError(
                "No se puede etiquetar una clase cancelada. "
                "Restáurala primero."
            )
    ...
    def cancelar(self, clase_id: str, docente_id: str,
                 observaciones: str) -> dict:
        if not observaciones or not observaciones.strip():
            raise ValueError(
                "Las observaciones son obligatorias al cancelar una clase"
            )
        clase = self._obtener_y_verificar(clase_id, docente_id)
        if clase["cancelada"]:
    ...
    def crear_extraordinaria(self, docente_id: str, grupo_id: str,
        ...
        # Verificar que el grupo le pertenece al docente
        # (reutilizamos la BD a través de una consulta directa)
        ...
        if self.repo.existe_clase_en_horario(grupo_id, fecha, hora_inicio):
            raise ValueError(
                "Ya existe una clase para ese grupo en esa fecha y hora"
            )
    ...
    # ------------------------------------------------------------------
    # US: Ver asistencias de una clase (tiempo real)
    # ------------------------------------------------------------------

    def asistencias_clase(self, clase_id: str,
                           docente_id: str) -> dict:
        clase = self._obtener_y_verificar(clase_id, docente_id)
